...
#     try:
#         dob = datetime.strptime(dob_str, "%Y-%m-%d").date()  # Expecting YYYY-MM-DD
#     except ValueError:
#         return jsonify({"error": "Invalid date format. Use YYYY-MM-DD"}), 400

#     new_child = Child(name=name, dob=dob, parent_id=parent_id)
...
#     children_data = [
#         {"id": c.id, "name": c.name, "dob": c.dob.strftime("%Y-%m-%d")}
#         for c in children
#     ]
...
#     if "dob" in data:
#         try:
#             child.dob = datetime.strptime(data['dob'], "%Y-%m-%d").date()
#         except ValueError:
