...
export default function DoctorDashboard() {
  ...
  const fetchPatients = async () => {
    ...
    try {
      const patientsWithReports = await doctorPatientApi.getAllPatientsWithReports();
      setPatients(patientsWithReports); // photoUrl can come from API
  ...
  const handleDownloadPDF = async (patient: Child) => {
    try {
      ...
      const reportData = {
        ...
        predictedClass: latestReport.predicted_class,
        reportId: latestReport.id,
        photoUrl: patient.photoUrl, // --- 2. Include photoUrl in downloadable data ---
      };
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
            ...
            {isLoading ? (
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {patients.map((patient) => (
                  <PatientCard
                    key={patient.id}
                    patient={patient} // photoUrl included in patient object
                    onViewDetails={handleViewDetails}
                    onDownloadPDF={handleDownloadPDF}
                    onSchedule={handleSchedule}
                  />
        ...
        {/* Recent Assessment Activity unchanged */}
