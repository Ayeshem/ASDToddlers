...
const API_BASE_URL =
  ...
  'https://lauderdale-pieces-leaders-max.trycloudflare.com';
  ...
  async getPatientWithLatestReport(childId: string): Promise<Child & { latestReport?: Report }> {
    try {
      ...
      return {
        ...
        riskLevel: reportData ? this.mapRiskLevel(reportData.risk_level) : undefined,
        latestReport: reportData || undefined,
        photoUrl: childData.photo ? `${API_BASE_URL}${childData.photo}` : undefined,
      };
    } catch (error) {
      console.error(`Failed to get patient with latest report for child ${childId}:`, error);
      throw error;
        ...
        children.map(async (child) => {
          try {
            ...
            return {
              ...
              riskLevel: this.mapRiskLevel(latestReport.risk_level),
              latestReport,
              photoUrl: child.photo ? `${API_BASE_URL}${child.photo}` : undefined,
            };
          } catch (error) {
            return {
              ...child,
      ...
      return patientsWithReports
        .filter(
          (result): result is PromiseFulfilledResult<
            Child & { latestReport?: Report; riskLevel?: 'safe' | 'low' | 'moderate' | 'high'; photoUrl?: string }
